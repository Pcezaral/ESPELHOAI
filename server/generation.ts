...
export async function generateTransformation(
  ...
  const themePrompts: Record<typeof theme, { prompt: string; text: string }> = {
    ...
    epic: {
      prompt: "IMPORTANT: Transform EVERY SINGLE person in the photo into epic ancient warriors/goddesses. Count all people and make sure EACH ONE gets warrior/goddess transformation - do not leave anyone unchanged. Choose ONE culture randomly: Greek (toga, laurel crown, goddess gown OR muscular warrior with bronze armor), Roman (senator robes, golden jewelry OR centurion armor with red cape), Viking (fur cloaks, horned helmets, braids OR valkyrie armor). Make ALL people look BEAUTIFUL and POWERFUL - enhance attractiveness, add dramatic lighting, heroic poses. Women: sensual elegant goddess/warrior attire. Men: muscular powerful warrior look. CRITICAL: Keep faces identical to original - same features, expressions. ALL people must be warriors/goddesses. Epic cinematic style.",
