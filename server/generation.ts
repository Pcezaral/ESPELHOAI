...
export async function generateTransformation(
  ...
  const themePrompts: Record<typeof theme, { prompt: string; text: string }> = {
    ...
    gangster: {
      // Gangster: ROSTO 100% IDÉNTICO, ALTA VARIEDADE de estilos (1920s, moderno, Tarantino-style)
      prompt: `Gangster/Crime Boss transformation. CRITICAL RULE: Keep EXACT same number of people - if 2 people in input, output MUST have 2 people with BOTH faces pixel-perfect identical; if 3 people, output MUST have 3 people with ALL 3 faces pixel-perfect identical. PRESERVE EACH INDIVIDUAL FACE: Each person's face must be PIXEL-PERFECT IDENTICAL - same eyes, nose, mouth, expression, age, skin tone, facial structure. DO NOT modify face. ONLY change: costume, pose, props, background, era. Randomly choose ONE scenario from different eras and styles: CLASSIC 1920s ERA (Speakeasy Boss in pinstripe suit + fedora + cigar + behind bar with whiskey bottles + Art Deco interior, Tommy Gun Shootout in suit + suspenders + fedora + holding tommy gun + brick alley + vintage car, Vintage Car Getaway driving 1920s Ford Model A + fedora + leather jacket + city street + motion blur, Jazz Club Owner in elegant suit + bow tie + jazz stage + musicians + smoky atmosphere, Casino High Roller in tuxedo + fedora + poker table + chips + chandelier + 1920s casino, Bootlegger Deal in trench coat + fedora + whiskey crates + dark warehouse + dramatic lighting, Mob Meeting in three-piece suit + fedora + round table + maps + money + cigar smoke, Bank Heist in suit + fedora + money bags + vault door + dramatic shadows, Rooftop Lookout in overcoat + fedora + binoculars + 1920s city skyline + neon signs). MODERN CRIME BOSS (Sharp tailored suit + sunglasses + modern penthouse office + city skyline windows + contemporary art, Nightclub Owner in designer suit + gold chains + VIP lounge + neon lights + modern luxury, Street Hustler in oversized hoodie + snapback + gold chains + urban street background + graffiti, Cartel Leader in tactical vest + sunglasses + luxury villa + armed guards silhouettes, Corrupt Politician in expensive suit + tie + government building + power pose + mahogany office). TARANTINO STYLE (Pulp Fiction style in sharp suit + sunglasses + diner booth + retro 90s background + cigarette smoke, Kill Bill Assassin in all black outfit + sword + dramatic action pose + Japanese temple background, Inglourious Bastards style in military-inspired outfit + dramatic lighting + war setting, Django Unchained style in period costume + dramatic pose + plantation background + western elements, Reservoir Dogs style in dark suit + sunglasses + warehouse + dramatic shadows + blood-splatter aesthetic). VARIED POSES: sitting at desk with gun + standing with arms crossed + leaning against car + walking down street + sitting in nightclub booth + standing in warehouse + driving luxury car + sitting at poker table. CRITICAL: NO sitting with whisky and cigar ONLY - vary poses and scenarios dramatically. Cinematic film noir or modern crime drama style. Dramatic lighting from different angles (harsh shadows / neon glow / spotlight / natural window light). Vary camera angle (low angle power shot / side profile / action shot / portrait / wide establishing shot). Realistic cinematic photo style. Random seed: ${randomSeed}, variation: ${randomVariation}`,
