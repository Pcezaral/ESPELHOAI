...
export async function generateTransformation(
  ...
  const themePrompts: Record<typeof theme, { prompt: string; text: string }> = {
    ...
    reveillon: {
      ...
      prompt: `New Year's Eve transformation. CRITICAL RULE: Keep EXACT same number of people - if 1 person in input, output MUST have 1 person; if 2 people, output MUST have 2 people with BOTH faces pixel-perfect identical. PRESERVE EACH INDIVIDUAL FACE: Face must be PIXEL-PERFECT IDENTICAL - same eyes, nose, mouth, expression, age, skin tone, facial structure. DO NOT modify face. ONLY change: costume, pose, props, background. Randomly choose ONE New Year scenario: Beach Celebration (elegant white flowing dress or white linen suit, holding champagne glass, beach at night with spectacular colorful fireworks exploding in sky, ocean reflecting lights, festive beach party atmosphere, string lights), Elegant Dinner Party (formal white suit or white elegant gown, bow tie or jewelry, holding champagne flute, upscale restaurant with golden balloons spelling year numbers, candles on table, champagne bottles, confetti, warm golden lighting), Rooftop Party (stylish white outfit, champagne glass, city rooftop with fireworks in background, urban skyline, party lights, celebratory atmosphere, night sky), Yacht Celebration (white nautical elegant outfit, champagne, luxury yacht deck, fireworks over water, festive maritime setting, glamorous night party). CRITICAL: White clothing is MANDATORY for all scenarios (Brazilian New Year tradition). Add fireworks, champagne, golden decorations, confetti. Vary pose (toasting / celebrating / elegant stance / joyful expression). Night lighting with fireworks glow and golden party lights. Photorealistic celebratory style. Random seed: ${randomSeed}`,
